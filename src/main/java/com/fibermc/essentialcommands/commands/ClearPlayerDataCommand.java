...
public class ClearPlayerDataCommand implements Command<CommandSourceStack> {
    ...
    public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (!CONFIG.ENABLE_DELETE_ALL_PLAYER_DATA) {
            source.sendFailure(Component.literal("This command is not enabled"));
            return -1;
        }

        var server = source.getServer();

        int currentTicks = server.getTickCount();
        int lastExecTime;
        if (source.isPlayer()) {
            var playerId = source.getPlayerOrException().getUUID();
