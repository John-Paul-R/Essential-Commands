...
public class JoinpointLimit {
    ...
    /**
     * Get the limit for a specific joinpoint type and permission level
     * @param type The joinpoint type
     * @param permissionLevel The player's permission level (0-based index)
     * @return The limit, or fallback to ANY type if specific type not found
     */
    public int getLimit(JoinpointType type, int permissionLevel) {
        List<Integer> typeLimits = limits.get(type);
        
        // If specific type not found, fallback to ANY
        if (typeLimits == null) {
            typeLimits = limits.get(JoinpointType.ANY);
        }
        
        // If still no limits found, return 0
        if (typeLimits == null || typeLimits.isEmpty()) {
            return 0;
        }
        
        // Return limit for permission level, or highest available limit
        if (permissionLevel >= typeLimits.size()) {
            return typeLimits.get(typeLimits.size() - 1);
        }
        
        return typeLimits.get(permissionLevel);
    }
    
    /**
     * Get the total limit for any type of joinpoint
     */
    public int getTotalLimit(int permissionLevel) {
        int total = 0;
        
        // If ANY is specified, that's the total limit
        if (limits.containsKey(JoinpointType.ANY)) {
            return getLimit(JoinpointType.ANY, permissionLevel);
        }
        
        // Otherwise sum up all specific limits
        for (JoinpointType type : JoinpointType.values()) {
            if (type != JoinpointType.ANY && limits.containsKey(type)) {
                total += getLimit(type, permissionLevel);
            }
        }
